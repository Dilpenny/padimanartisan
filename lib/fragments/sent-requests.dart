...
import 'customer-request-detail.dart';
...
class SentRequestsPage extends StatefulWidget {
  const SentRequestsPage({Key? key, this.tag}) : super(key: key);
  final tag; // fetch only this slug
  @override
  SentRequestsPageWidget createState() => SentRequestsPageWidget(tag);
}

class SentRequestsPageWidget extends State<SentRequestsPage> {
  final tag; // fetch only this slug
  int page = 1;
  int processing = 0;
  int total = 0;
  ...
  SentRequestsPageWidget(this.tag);
    ...
    return FocusDetector(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          elevation: 0,
          title: Text('Sent requests', style: GoogleFonts.quicksand()),
        ),
        backgroundColor: whiteColor,
        body: Container(
    ...
    return GestureDetector(
      onTap: (){
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) =>
                CustomerRequestDetailPage(requestObj: obj),
          ),
        );
      },
      child: Container(
          ...
          child: ListTile(
              ...
              title: Column(
                ...
                children: <Widget>[
                  Wrap(
                    children: <Widget>[
                      Text(obj.artisan!,style: TextStyle(fontSize: 15,color: Colors.grey.shade600),),
  ...
  Future<List<CustomerRequest>> _fetchJobs() async {
    try{
      var session = FlutterSession();
      int userId = await session.getInt('id');
      String basicUrl = '${Component().API}mobile/my/requests?user_id=$userId';
